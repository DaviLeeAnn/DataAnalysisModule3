...
select name from products where name like "%latte%";
...
select orders.order_id, orders.status, order_items.quantity as total_items
from orders
inner join order_items on orders.order_id=order_items.quantity;
